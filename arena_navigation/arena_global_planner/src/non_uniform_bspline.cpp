...
void NonUniformBspline::setUniformBspline(const Eigen::MatrixXd& points, const int& order,
  ...
  n_ = points.rows() - 1;   //
  m_ = n_ + p_ + 1;         // t0,t1,t2,...,tm, here is u0 u1 ... um
...
std::pair<Eigen::VectorXd, Eigen::VectorXd> NonUniformBspline::getHeadTailPts() {
...
void NonUniformBspline::parameterizeToBspline(const double& ts, const std::vector<Eigen::Vector2d>& point_set,
