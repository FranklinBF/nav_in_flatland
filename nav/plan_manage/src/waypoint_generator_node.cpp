...
bool WaypointGenerator::get_next_subgoal()
{
    ...
    for (int i = _visted_waypoint_id; i < _global_plan.poses.size(); i++)
    {
        ...
        if ((_wpNodes.at(i)->dist_to_robot > _look_ahead_distance - _tolerance) && (_wpNodes.at(i)->dist_to_robot < _look_ahead_distance + _tolerance))
        {
...
int main(int argc, char **argv)
{
    ...
    ROS_ERROR("distance %s %.1f", param_look_ahead_distance.c_str(), look_ahead_distance);
    ...
    ROS_ERROR("param_waypoint_tolerance %s %.1f", param_waypoint_tolerance.c_str(), waypoint_tolerance);
